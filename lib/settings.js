...
class Settings {
  static async syncAll (nop, context, repo, config, ref) {
    ...
    try {
      ...
      settings.logError(`An error occured during syncAll: ${error.message}, stacktrace: ${error.stacktrace}`)
      await settings.handleResults()
    }
  }
  ...
  static async syncSubOrgs (nop, context, suborg, repo, config, ref) {
    ...
    try {
      ...
      settings.logError(`An error occured during syncSubOrgs: ${error.message}, stacktrace: ${error.stacktrace}`)
      await settings.handleResults()
    }
  }
  ...
  static async sync (nop, context, repo, config, ref) {
    ...
    try {
      ...
      settings.logError(`An error occured during sync: ${error.message}, stacktrace: ${error.stacktrace}`)
      await settings.handleResults()
    }
  }
  ...
  static async syncTeams (nop, context, repo, config, ref) {
    ...
    try {
      ...
      settings.logError(`An error occured during syncTeams: ${error.message}, stacktrace: ${error.stacktrace}`)
      await settings.handleResults()
    }
  }
...
`
  ...
  async updateOrgTeams (org, teamSlug) {
  ...
  isRestricted (name, configKey = 'restrictedRepos') {
    ...
    if (Array.isArray(restricted)) {
      ...
      if (restricted.includes(name)) {
        this.log.debug(`Skipping ${configKey} ${name}`)
        return true
      } else {
