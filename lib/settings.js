  ...
  async handleResults () {
    ...
    */
    ...
    if (env.ENABLE_PR_COMMENT === 'true') {
      ...
      const quietResults = this.results.filter(result => !!result?.isQuiet).map(
        result => ({
...
<sub>${quietResults.length} NOOP actions are included in the source of this comment</sub>

<!--
${stripAllWhitespace(JSON.stringify(quietResults))}
-->
`
    ...
    const params = {
      ...
      output: {
        ...
        summary: truncateForGithub(stripLeadingWhitespace(commentmessage))
      }
    }
...
const stripLeadingWhitespace = (input) => input.replace(/\n(\s+)/g, '\n')

const stripAllWhitespace = (input) => input.replace(/\s/g, '')

const truncateForGithub = (input, message = '...[TRUNCATED]') => {
  ...
  return input.substr(0, maxLength - message.length) + message
}
