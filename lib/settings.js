...
class Settings {
  ...
  async createCheckRun () {
    ...
    return meteredPlugin(this, () => this.github.rest.repos.listCommits({
...
`
      ...
      await meteredPlugin(this, () => this.github.rest.issues.createComment({
    ...
    await meteredPlugin(this, () => this.github.rest.checks.update(params))
  }
    ...
    if (repoConfig) {
      try {
        ...
        if (isArchived && !shouldUnarchive) {
          this.log.debug(`Repo ${repo.repo} is archived. Skipping...`)
          return
        }
