...
module.exports = class Collaborators extends Diffable {
  ...
  find () {
    ...
    return Promise.all([this.reposApi.listCollaborators({ repo: this.repo.repo, owner: this.repo.owner, affiliation: 'direct' }),
      this.reposApi.listInvitations({ repo: this.repo.repo, owner: this.repo.owner })])
      .then(res => {
  ...
  add (attrs) {
    ...
    if (this.nop) {
      return Promise.resolve([
        new NopCommand(this.constructor.name, this.repo, this.reposApi.addCollaborator.endpoint(data), 'Add Collaborators')
      ])
    }
    return meteredPlugin(this, () => this.reposApi.addCollaborator(data))
  }
  ...
  updateInvite (invitation_id, permissions) {
    ...
    if (this.nop) {
      return Promise.resolve([
        new NopCommand(this.constructor.name, this.repo, this.reposApi.updateInvitation.endpoint(data), 'Update Invitation')
      ])
    }
    return meteredPlugin(this, () => this.reposApi.updateInvitation(data))
  }
  ...
  remove (existing) {
    if (existing.pendinginvite) {
      ...
      if (this.nop) {
        return Promise.resolve([
          new NopCommand(this.constructor.name, this.repo, this.reposApi.deleteInvitation.endpoint(data),
            'Delete Invitation')
        ])
      }
      return meteredPlugin(this, () => this.reposApi.deleteInvitation(data))
    } else {
      const data = Object.assign({ username: existing.username }, this.repo)
      if (this.nop) {
        return Promise.resolve([
          new NopCommand(this.constructor.name, this.repo, this.reposApi.removeCollaborator.endpoint(data),
            'Remove Collaborator')
        ])
      }
      return meteredPlugin(this, () => this.reposApi.removeCollaborator(data))
    }
  }
}
