...
module.exports = class Milestones extends Diffable {
  ...
  update (existing, attrs) {
    ...
    return meteredPlugin(this, () => this.github.rest.issues.updateMilestone(Object.assign({ milestone_number: existing.number }, attrs, { owner, repo })))
  }

  add (attrs) {
    const { owner, repo } = this.repo

    return meteredPlugin(this, () => this.github.rest.issues.createMilestone(Object.assign({}, attrs, { owner, repo })))
  }

  remove (existing) {
    const { owner, repo } = this.repo

    return meteredPlugin(this, () => this.github.rest.issues.deleteMilestone(Object.assign({ milestone_number: existing.number }, { owner, repo })))
  }
}
