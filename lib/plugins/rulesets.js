...
module.exports = class Rulesets extends Diffable {
  ...
  update (existing, attrs) {
    ...
    if (this.scope === 'org') {
      ...
      return meteredPlugin(this, () => this.github.request('PUT /orgs/{org}/rulesets/{id}', parms)).then(res => {
      ...
      Overrides.removeOverrides(overrides, parms, existing)
      this.log.debug(`Updating Ruleset with the following values ${JSON.stringify(parms, null, 2)}`)
      return meteredPlugin(this, () => this.github.request('PUT /repos/{owner}/{repo}/rulesets/{id}', parms)).then(res => {
        this.log.debug(`Ruleset updated successfully ${JSON.stringify(res.url)}`)
        return res
      }).catch(e => {
        return this.handleError(e)
  ...
  add (attrs) {
    if (this.scope === 'org') {
      ...
      return meteredPlugin(this, () => this.github.request('POST /orgs/{org}/rulesets', this.wrapAttrs(attrs))).then(res => {
      ...
      Overrides.removeOverrides(overrides, attrs, {})
      this.log.debug(`Creating Rulesets with the following values ${JSON.stringify(attrs, null, 2)}`)
      return meteredPlugin(this, () => this.github.request('POST /repos/{owner}/{repo}/rulesets', this.wrapAttrs(attrs))).then(res => {
        this.log.debug(`Ruleset created successfully ${JSON.stringify(res.url)}`)
        return res
      }).catch(e => {
        return this.handleError(e)
  ...
  remove (existing) {
    ...
    if (this.scope === 'org') {
      ...
      return meteredPlugin(this, () => this.github.request('DELETE /orgs/{org}/rulesets/{id}', parms)).then(res => {
      ...
      this.log.debug(`Deleting Ruleset with the following values ${JSON.stringify(parms, null, 2)}`)
      return meteredPlugin(this, () => this.github.request('DELETE /repos/{owner}/{repo}/rulesets/{id}', parms)).then(res => {
        this.log.debug(`Ruleset deleted successfully ${JSON.stringify(res.url)}`)
        return res
      }).catch(e => {
        if (e.status === 404) {
