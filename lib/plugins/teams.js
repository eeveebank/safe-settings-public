    ...
    return this.github.paginate(this.github.repos.listTeams, this.repo)
      .then(teams =>
        Promise.all(
          ...
          this.log.debug(`teamsWithRoleNames ${JSON.stringify(teamsWithRoleNames)}`)
          return teams.map((team, i) => ({
            ...team,
            permission: getCustomRoleNameOrPermission(teamsWithRoleNames[i]?.data?.role_name, team.permission)
          }))
        }).catch(e => {
...
const defaultRoles = ['read', 'triage', 'write', 'maintain', 'admin']

const getCustomRoleNameOrPermission = (roleName, permission) => {
  if (roleName && !defaultRoles.includes(roleName)) {
    return roleName
  }
