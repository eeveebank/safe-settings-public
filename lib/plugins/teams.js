...
module.exports = class Teams extends Diffable {
  async find () {
    ...
    return this.github.paginate(this.github.repos.listTeams, this.repo)
      .then(teams =>
        Promise.all(
          teams.map(
            team => this.github.request(`GET ${teamRepoEndpoint}`, {
              team_id: team.id,
              owner: this.repo.owner,
              repo: this.repo.repo
            })
          )
        ).then(teamsWithRoleNames =>
          teams.map((team, i) => ({
            ...team,
            role_name: teamsWithRoleNames[i].role_name
          }))
        ).catch(e => {
          this.log.error(`Error fetching team with role_name ${e}`)
          throw e
        }))
  }
  ...
  changed (existing, attrs) {
    if (isCustomRole(attrs.permission)) {
      return existing.role_name !== attrs.permission
    }
...
// TODO: Future proof - read these values from the keys of a team's `permissions` property
const permissions = ['admin', 'pull', 'push', 'maintain', 'triage']

const isCustomRole = (permissionOrRole) => !permissions.includes(permissionOrRole)
