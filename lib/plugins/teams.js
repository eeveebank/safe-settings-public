...
module.exports = class Teams extends Diffable {
  constructor (...args) {
    super(...args)
    this.reposApi = this.github.rest?.repos ?? this.github.repos
    this.teamsApi = this.github.rest?.teams ?? this.github.teams
  }

  async find () {
    this.log.debug(`Finding teams for ${this.repo.owner}/${this.repo.repo}`)
    return this.github.paginate(this.reposApi.listTeams, this.repo).then(res => {
  ...
  async checkSecurityManager (teams) {
    try {
      if (typeof this.github.request.endpoint !== 'function') {
        return teams
      }
      ...
      const securityManagersEndpoint = this.github.request.endpoint
        ? JSON.stringify(this.github.request.endpoint('GET /orgs/{org}/security-managers', { org: this.repo.owner }))
        : 'GET /orgs/{org}/security-managers'
      this.log.debug(`Calling API to get security managers ${securityManagersEndpoint} `)
  ...
  add (attrs) {
    ...
    return this.teamsApi.getByName({ org: this.repo.owner, team_slug: attrs.name }).then(res => {
      existing = res.data
      this.log.debug(`adding team ${attrs.name} to repo ${this.repo.repo}`)
      if (this.nop) {
        return Promise.resolve([
          new NopCommand(this.constructor.name, this.repo, this.teamsApi.addOrUpdateRepoPermissionsInOrg.endpoint(this.toParams(existing, attrs)), 'Add Teams to Repo')
        ])
      }
      return meteredPlugin(this, () => this.teamsApi.addOrUpdateRepoPermissionsInOrg(this.toParams(existing, attrs))).then(res => {
      ...
      if (e.status === 404) {
        ...
        if (this.nop) {
          return Promise.resolve([
            new NopCommand(this.constructor.name, this.repo, this.teamsApi.create.endpoint(createParam), 'Create Team')
          ])
        }
        return meteredPlugin(this, async () => {
          const res = await this.teamsApi.create(createParam)
          this.log.debug(`team ${createParam.name} created`)
          existing = res.data
          this.log.debug(`adding team ${attrs.name} to repo ${this.repo.repo}`)
          return this.teamsApi.addOrUpdateRepoPermissionsInOrg(this.toParams(existing, attrs))
        }).catch(e => {
