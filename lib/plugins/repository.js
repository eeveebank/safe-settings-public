...
module.exports = class Repository extends ErrorStash {
    ...
    return this.github.rest.repos.getBranch({
      ...
      if (res.data.name !== newname) {
        ...
        if (this.nop) {
          ...
          return meteredPlugin(this, () => this.github.rest.repos.update(parms))
        }
      }
    }).catch(e => {
      if (e.status === 404) {
  ...
  updaterepo (resArray) {
    ...
    return meteredPlugin(this, () => this.github.rest.repos.update(this.settings))
  }
  ...
  updatetopics (repoData, resArray) {
    ...
    if (this.topics) {
      ...
      return meteredPlugin(this, () => this.github.rest.repos.replaceAllTopics(parms))
      // } else {
  ...
  updateSecurity (repoData, resArray) {
    if (this.security?.enableVulnerabilityAlerts === true || this.security?.enableVulnerabilityAlerts === false) {
      ...
      if (this.security.enableVulnerabilityAlerts === true) {
        ...
        return meteredPlugin(this, () => this.github.rest.repos.enableVulnerabilityAlerts({
          owner: repoData.owner.login,
          repo: repoData.name
        }))
      } else {
        ...
        return meteredPlugin(this, () => this.github.rest.repos.disableVulnerabilityAlerts({
          owner: repoData.owner.login,
          repo: repoData.name
        }))
      }
  ...
  updateAutomatedSecurityFixes (repoData, resArray) {
    if (this.security?.enableAutomatedSecurityFixes === true || this.security?.enableAutomatedSecurityFixes === false) {
      if (this.security.enableAutomatedSecurityFixes === true) {
        ...
        return meteredPlugin(this, () => this.github.rest.repos.enableAutomatedSecurityFixes({
          owner: repoData.owner.login,
          repo: repoData.name
        }))
      } else {
        ...
        return meteredPlugin(this, () => this.github.rest.repos.disableAutomatedSecurityFixes({
          owner: repoData.owner.login,
          repo: repoData.name
        }))
      }
